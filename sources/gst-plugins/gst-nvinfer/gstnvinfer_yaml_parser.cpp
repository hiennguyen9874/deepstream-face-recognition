...
static gboolean gst_nvinfer_parse_other_attribute_yaml(GstNvInfer *nvinfer,
                                                       const gchar *cfg_file_path,
...
static gboolean gst_nvinfer_parse_props_yaml(GstNvInfer *nvinfer,
                                             ...
                                             const gchar *cfg_file_path)
{
    ...
    for (YAML::const_iterator itr = configyml["property"].begin();
        ...
        if (paramKey == "gie-unique-id") {
            ...
            if (!get_absolute_file_path(cfg_file_path, temp.c_str(), init_params->labelsFilePath)) {
            ...
            init_params->inferInputDims =
                NvDsInferDimsCHW{(unsigned int)std::stoul(vec[0]), (unsigned int)std::stoul(vec[1]),
            ...
            if (!get_absolute_file_path(cfg_file_path, temp.c_str(), init_params->customLibPath)) {
            ...
            if (!get_absolute_file_path(cfg_file_path, temp.c_str(), init_params->modelFilePath)) {
            ...
            if (!get_absolute_file_path(cfg_file_path, temp.c_str(), init_params->protoFilePath)) {
            ...
            if (!get_absolute_file_path(cfg_file_path, temp.c_str(), init_params->uffFilePath)) {
            ...
            if (!get_absolute_file_path(cfg_file_path, temp.c_str(), init_params->onnxFilePath)) {
            ...
            default:
                g_printerr(
                    "Error. Invalid value for segmentation-output-order, Segmentation output order "
                    ":'%d'\n",
                    val);
                goto done;
                break;
            ...
            if (!gst_nvinfer_parse_other_attribute_yaml(nvinfer, cfg_file_path, values)) {
                goto done;
            }
        }
    }
                                            ...
                                            const gchar *cfg_file_path)
{
    ...
    if (init_params->networkType == NvDsInferNetworkType_Detector ||
        ...
        if (configyml["class-attrs-all"]) {
            ...
            if (!gst_nvinfer_parse_class_attrs(cfg_file_path, temp, detection_params,
                                               detection_filter_params, color_params)) {
