...
DsNvInferImpl::ModelLoadThread::ModelLoadThread(DsNvInferImpl &impl) : m_Impl(impl)
{
...
bool DsNvInferImpl::triggerNewModel(const std::string &modelPath, ModelLoadType loadType)
{
...
void DsNvInferImpl::loadModel(const std::string &modelPath, ModelLoadType loadType)
{
    ...
    if (!initNewInferModelParams(newParams, modelPath, loadType, curParam)) {
        notifyLoadModelStatus(ModelStatus{NVDSINFER_CONFIG_FAILED, modelPath,
    ...
    NvDsInferStatus err =
        createNvDsInferContext(&newContext, newParams, m_GstInfer, gst_nvinfer_logger);

    if (err != NVDSINFER_SUCCESS) {
        /* Notify application if the model load failed. */
        notifyLoadModelStatus(ModelStatus{err, modelPath, "Creation new model context failed"});
        return;
    }
    ...
    if (!triggerContextReplace(newCtxPtr, std::move(newParamsPtr), modelPath)) {
        notifyLoadModelStatus(
            ModelStatus{NVDSINFER_UNKNOWN_ERROR, modelPath, "trigger new model replace failed"});
        return;
    }
}
                                            ...
                                            const NvDsInferContextInitParams &oldParams)
{
    ...
    case MODEL_LOAD_FROM_CONFIG:
        if (!gst_nvinfer_parse_context_params(&newParams, newModelPath.c_str())) {
            GST_WARNING_OBJECT(m_GstInfer, "[UID %d]: parse new model config file: %s failed.",
    ...
    if (oldParams.numDetectedClasses) {
        ...
        memcpy(newParams.perClassDetectionParams, oldParams.perClassDetectionParams,
               sizeof(newParams.perClassDetectionParams[0]) * newParams.numDetectedClasses);
    }
                                      ...
                                      NvDsInferContextInitParams &newParam)
{
    if (newParam.maxBatchSize < m_GstInfer->max_batch_size) {
        GST_WARNING_OBJECT(
            ...
            m_GstInfer->unique_id, newParam.maxBatchSize, m_GstInfer->max_batch_size);
        return false;
    }
...
void DsNvInferImpl::notifyLoadModelStatus(const ModelStatus &res)
{
    ...
    if (res.status == NVDSINFER_SUCCESS) {
        ...
        GST_ELEMENT_WARNING(
            ...
            ("[UID %d]: Load new model:%s failed, reason: %s", m_GstInfer->unique_id,
             res.cfg_file.c_str(), (res.message.empty() ? "unknown" : res.message.c_str())),
            (nullptr));
    }
    g_signal_emit(m_GstInfer, gst_nvinfer_signals[SIGNAL_MODEL_UPDATED], 0, (int)res.status,
                  res.cfg_file.c_str());
}
 ...
 * the model. */
NvDsInferStatus DsNvInferImpl::resetContextUnlock(NvDsInferContextPtr ctx,
                                                  NvDsInferContextInitParamsPtr params,
                                                  const std::string &path)
{
...
NvDsInferStatus DsNvInferImpl::ensureReplaceNextContext()
{
    ...
    if (err != NVDSINFER_SUCCESS) {
        notifyLoadModelStatus(ModelStatus{err, path, "Model update failed while flushing data"});
        return err;
    }
    ...
    if (err != NVDSINFER_SUCCESS) {
        notifyLoadModelStatus(
            ModelStatus{err, path, "Model update failed while replacing the current context"});
        return err;
    }

    /* Notify application of successful model load. */
    notifyLoadModelStatus(ModelStatus{NVDSINFER_SUCCESS, path, "New Model updated succefully"});
    return NVDSINFER_SUCCESS;
}
