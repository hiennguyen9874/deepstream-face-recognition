...
static void gst_nvinfer_set_property(GObject *object,
                                     guint prop_id,
                                     const GValue *value,
                                     GParamSpec *pspec);
static void gst_nvinfer_get_property(GObject *object,
                                     guint prop_id,
                                     GValue *value,
                                     GParamSpec *pspec);
...
static GType gst_nvinfer_process_mode_get_type(void)
{
    ...
    if (g_once_init_enter(&process_mode_type)) {
        GType tmp = g_enum_register_static("GstNvInferProcessModeType", process_mode);
...
static void gst_nvinfer_class_init(GstNvInferClass *klass)
{
    ...
    gstbasetransform_class->sink_event = GST_DEBUG_FUNCPTR(gst_nvinfer_sink_event);

    gstbasetransform_class->submit_input_buffer =
        GST_DEBUG_FUNCPTR(gst_nvinfer_submit_input_buffer);
    gstbasetransform_class->generate_output = GST_DEBUG_FUNCPTR(gst_nvinfer_generate_output);
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            ...
            "Path to the configuration file for this instance of nvinfer", DEFAULT_CONFIG_FILE_PATH,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean(
            "raw-output-file-write", "Raw Output File Write", "Write raw inference output to file",
    ...
    gst_nvinfer_signals[SIGNAL_MODEL_UPDATED] =
        g_signal_new("model-updated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(GstNvInferClass, model_updated), NULL, NULL, NULL, G_TYPE_NONE,
    ...
    gst_element_class_set_details_simple(gstelement_class, "NvInfer plugin", "NvInfer Plugin",
                                     ...
                                     GParamSpec *pspec)
{
    ...
    switch (prop_id) {
    case PROP_UNIQUE_ID:
        impl->m_InitParams->uniqueID = nvinfer->unique_id = g_value_get_uint(value);
        break;
    ...
    case PROP_BATCH_SIZE:
        nvinfer->max_batch_size = impl->m_InitParams->maxBatchSize = g_value_get_uint(value);
        break;
    case PROP_INTERVAL:
    ...
    case PROP_MODEL_ENGINEFILE: {
        ...
        g_strlcpy(impl->m_InitParams->modelEngineFilePath, g_value_get_string(value), _PATH_MAX);
    } break;
                                     ...
                                     GParamSpec *pspec)
{
    ...
    case PROP_PROCESS_MODE:
        g_value_set_enum(
            value, nvinfer->process_full_frame ? PROCESS_MODEL_FULL_FRAME : PROCESS_MODEL_OBJECTS);
        break;
    case PROP_CONFIG_FILE_PATH:
...
static void gst_nvinfer_reset_init_params(GstNvInfer *nvinfer)
{
    ...
    if (nvinfer->is_prop_set->at(PROP_MODEL_ENGINEFILE))
        g_strlcpy(impl->m_InitParams->modelEngineFilePath, prev_params->modelEngineFilePath,
                  _PATH_MAX);
...
static gboolean gst_nvinfer_start(GstBaseTransform *btrans)
{
    ...
    if (!nvinfer->config_file_path || strlen(nvinfer->config_file_path) == 0) {
        GST_ELEMENT_ERROR(nvinfer, LIBRARY, SETTINGS, ("Configuration file not provided"),
                          (nullptr));
        return FALSE;
    }
    if (nvinfer->config_file_parse_successful == FALSE) {
        GST_ELEMENT_ERROR(nvinfer, LIBRARY, SETTINGS, ("Configuration file parsing failed"),
    ...
    if (nvinfer->output_instance_mask == TRUE &&
        ...
        GST_ELEMENT_ERROR(
            ...
            ("Instance mask output not supported with cluster mode %d", init_params->clusterMode),
            (nullptr));
        return FALSE;
    }
    ...
    init_params->copyInputToHostBuffers =
        (nvinfer->write_raw_buffers_to_file || (nvinfer->output_generated_callback != nullptr));
    ...
    if (status != NVDSINFER_SUCCESS) {
        GST_ELEMENT_ERROR(nvinfer, RESOURCE, FAILED, ("Failed to create NvDsInferContext instance"),
    ...
    default:
        GST_ELEMENT_ERROR(nvinfer, LIBRARY, SETTINGS,
                          ("Unsupported network input format: %d", init_params->networkInputFormat),
                          (nullptr));
        return FALSE;
    }
    ...
    cudaReturn = cudaStreamCreateWithFlags(&nvinfer->convertStream, cudaStreamNonBlocking);
    if (cudaReturn != cudaSuccess) {
        GST_ELEMENT_ERROR(
            nvinfer, RESOURCE, FAILED, ("Failed to create cuda stream"),
    ...
    nvinfer->source_info->emplace(0, GstNvInferSourceInfo{GstNvInferObjectHistoryMap(), 0});
    ...
    impl->notifyLoadModelStatus(
        ModelStatus{NVDSINFER_SUCCESS, nvinfer->config_file_path, "Model loaded successfully"});
    return TRUE;
}
 ...
 * input format.
 */
static GstFlowReturn get_converted_buffer(GstNvInfer *nvinfer,
                                          ...
                                          void *destCudaPtr)
{
    ...
    nvinfer->transform_params.src_rect[nvinfer->tmp_surf.numFilled] = {src_top, src_left, src_width,
                                                                       src_height};
                                                 ...
                                                 NvBufSurface *in_surf)
{
    ...
    for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
            ...
            bool needs_infer =
                should_infer_object(nvinfer, inbuf, object_meta, frame_num, obj_history.get());
            if (!needs_infer) {
            ...
            if (obj_history && nvinfer->classifier_async_mode) {
                ...
                attach_metadata_classifier(nvinfer, nullptr, frame, obj_history->cached_info);
                obj_history->last_accessed_frame_num = frame_meta->frame_num;
            }
            ...
            if (source_info != nullptr && object_meta->object_id != UNTRACKED_OBJECT_ID &&
                ...
                auto ret_iter = source_info->object_history_map.emplace(
                    object_meta->object_id, std::make_shared<GstNvInferObjectHistory>());
                                                     ...
                                                     GstBuffer *inbuf)
{
    ...
    if (impl->ensureReplaceNextContext() != NVDSINFER_SUCCESS) {
        GST_ELEMENT_ERROR(nvinfer, RESOURCE, FAILED, ("Ensure next context failed."),
                          ("streaming stopped"));
        return GST_FLOW_ERROR;
    }
    ...
    if (nvinfer->classifier_async_mode) {
        ...
        if (nvinfer->last_flow_ret != flow_ret) {
            ...
            case GST_FLOW_NOT_NEGOTIATED:
                GST_ELEMENT_ERROR(
                    nvinfer, STREAM, FAILED, ("Internal data stream error."),
...
static void gst_nvinfer_output_generated_file_write(GstBuffer *buf,
                                                    ...
                                                    GstNvInfer *nvinfer)
{
    ...
    for (i = 0; i < num_layers; i++) {
        ...
        g_snprintf(file_name, 256, "gstnvdsinfer_uid-%02d_layer-%s_batch-%010lu_batchsize-%02d.bin",
                   nvinfer->unique_id, info->layerName, nvinfer->file_write_batch_num, batch_size);
        file_name[255] = '\0';
    ...
    while (!nvinfer->stop) {
        ...
        if (batch->push_buffer) {
            ...
            if (nvinfer->last_flow_ret != flow_ret) {
                ...
                case GST_FLOW_NOT_LINKED:
                case GST_FLOW_NOT_NEGOTIATED:
                    GST_ELEMENT_ERROR(nvinfer, STREAM, FAILED, ("Internal data stream error."),
        ...
        std::unique_ptr<GstNvInferTensorOutputObject, decltype(tensor_deleter)> tensor_out_object(
            new GstNvInferTensorOutputObject, tensor_deleter);
        ...
        if (nvinfer->output_generated_callback) {
            nvinfer->output_generated_callback(batch->inbuf, &nvinfer->network_info,
        ...
        for (auto &hist : batch->objs_pending_meta_attach) {
            ...
            frame.obj_meta = hist.second;
            std::weak_ptr<GstNvInferObjectHistory> obj_history = hist.first;
            attach_metadata_classifier(nvinfer, nullptr, frame, obj_history.lock()->cached_info);
        }
...
static gboolean nvinfer_plugin_init(GstPlugin *plugin)
{
    ...
    return gst_element_register(plugin, "nvinfer", GST_RANK_PRIMARY, GST_TYPE_NVINFER);
}
