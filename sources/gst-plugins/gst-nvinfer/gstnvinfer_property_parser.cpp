    ...
    GstNvInferColorParams &color_params)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_THRESHOLD)) {
            ...
            detection_params.preClusterThreshold = g_key_file_get_double(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_PRE_CLUSTER_THRESHOLD, &error);
            CHECK_ERROR(error);
            if (detection_params.preClusterThreshold < 0) {
            ...
            detection_params.postClusterThreshold = g_key_file_get_double(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_POST_CLUSTER_THRESHOLD, &error);
            CHECK_ERROR(error);
            ...
            detection_params.eps =
                g_key_file_get_double(key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_EPS, &error);
            CHECK_ERROR(error);
        ...
        } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_GROUP_THRESHOLD)) {
            detection_params.groupThreshold = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_GROUP_THRESHOLD, &error);
            CHECK_ERROR(error);
            if (detection_params.groupThreshold < 0) {
                g_printerr("Error: Negative group-threshold (%d) specified for group %s\n",
            ...
            detection_params.minScore = g_key_file_get_double(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DBSCAN_MIN_SCORE, &error);
            CHECK_ERROR(error);
            ...
            detection_filter_params.roiTopOffset = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_ROI_TOP_OFFSET, &error);
            CHECK_ERROR(error);
        ...
        } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_ROI_BOTTOM_OFFSET)) {
            detection_filter_params.roiBottomOffset = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_ROI_BOTTOM_OFFSET, &error);
            CHECK_ERROR(error);
            if ((gint)detection_filter_params.roiBottomOffset < 0) {
                g_printerr("Error: Negative roiBottomOffset (%d) specified for group %s\n",
                           detection_filter_params.roiBottomOffset, group);
                goto done;
            }
        } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_WIDTH)) {
            detection_filter_params.detectionMinWidth = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_WIDTH, &error);
            CHECK_ERROR(error);
            if ((gint)detection_filter_params.detectionMinWidth < 0) {
                g_printerr("Error: Negative detectionMinWidth (%d) specified for group %s\n",
                           detection_filter_params.detectionMinWidth, group);
                goto done;
            }
        } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_HEIGHT)) {
            detection_filter_params.detectionMinHeight = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_HEIGHT, &error);
            CHECK_ERROR(error);
            if ((gint)detection_filter_params.detectionMinHeight < 0) {
                g_printerr("Error: Negative detectionMinHeight (%d) specified for group %s\n",
                           detection_filter_params.detectionMinHeight, group);
                goto done;
            }
        } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_WIDTH)) {
            detection_filter_params.detectionMaxWidth = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_WIDTH, &error);
            CHECK_ERROR(error);
            if ((gint)detection_filter_params.detectionMaxWidth < 0) {
                g_printerr("Error: Negative detectionMaxWidth (%d) specified for group %s\n",
                           detection_filter_params.detectionMaxWidth, group);
                goto done;
            }
        } else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_HEIGHT)) {
            detection_filter_params.detectionMaxHeight = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_HEIGHT, &error);
            if ((gint)detection_filter_params.detectionMaxHeight < 0) {
                g_printerr("Error: Negative detectionMaxHeight (%d) specified for group %s\n",
            ...
            detection_params.nmsIOUThreshold = g_key_file_get_double(
                key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_NMS_IOU_THRESHOLD, &error);
            CHECK_ERROR(error);
                                                  ...
                                                  const gchar *cfg_file_path)
{
    ...
    if (!g_strcmp0(key, CONFIG_GROUP_INFER_PROCESS_MODE)) {
        if ((*nvinfer->is_prop_set)[PROP_PROCESS_MODE])
            return TRUE;
        guint val =
            g_key_file_get_integer(key_file, group_name, CONFIG_GROUP_INFER_PROCESS_MODE, &error);
        CHECK_ERROR(error);
        ...
        if (g_key_file_get_boolean(key_file, group_name, CONFIG_GROUP_INFER_CLASSIFIER_ASYNC_MODE,
                                   &error))
        ...
        if ((*nvinfer->is_prop_set)[PROP_INTERVAL])
            return TRUE;
        nvinfer->interval =
            g_key_file_get_integer(key_file, group_name, CONFIG_GROUP_INFER_INTERVAL, &error);
        CHECK_ERROR(error);
        ...
        if (g_key_file_get_boolean(key_file, group_name, CONFIG_GROUP_INFER_OUTPUT_TENSOR_META,
                                   &error))
        ...
        nvinfer->min_input_object_width = g_key_file_get_integer(
            key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MIN_WIDTH, &error);
        CHECK_ERROR(error);
        if ((gint)nvinfer->min_input_object_width < 0) {
            g_printerr("Error: Negative value specified for %s(%d)\n",
                       CONFIG_GROUP_INFER_INPUT_OBJECT_MIN_WIDTH, nvinfer->min_input_object_width);
            goto done;
        }
    } else if (!g_strcmp0(key, CONFIG_GROUP_INFER_INPUT_OBJECT_MIN_HEIGHT)) {
        nvinfer->min_input_object_height = g_key_file_get_integer(
            key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MIN_HEIGHT, &error);
        CHECK_ERROR(error);
        ...
        nvinfer->max_input_object_width = g_key_file_get_integer(
            key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_WIDTH, &error);
        CHECK_ERROR(error);
        if ((gint)nvinfer->max_input_object_width < 0) {
            g_printerr("Error: Negative value specified for %s(%d)\n",
                       CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_WIDTH, nvinfer->max_input_object_width);
            goto done;
        }
    } else if (!g_strcmp0(key, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_HEIGHT)) {
        nvinfer->max_input_object_height = g_key_file_get_integer(
            key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_HEIGHT, &error);
        CHECK_ERROR(error);
        ...
        int val =
            g_key_file_get_integer(key_file, group_name, CONFIG_GROUP_INFER_SCALING_FILTER, &error);
        CHECK_ERROR(error);
                                        ...
                                        const gchar *cfg_file_path)
{
    ...
    if (g_key_file_has_key(key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_IS_CLASSIFIER_LEGACY,
                           nullptr) &&
        !g_key_file_has_key(key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_NETWORK_TYPE,
                            nullptr)) {
        ...
        g_key_file_set_integer(key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_NETWORK_TYPE,
                               new_value);
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_INFER_UNIQUE_ID)) {
            if (nvinfer && (*nvinfer->is_prop_set)[PROP_UNIQUE_ID])
                continue;
            init_params->uniqueID = g_key_file_get_integer(key_file, CONFIG_GROUP_PROPERTY,
            ...
            init_params->gpuID = g_key_file_get_integer(key_file, CONFIG_GROUP_PROPERTY,
            ...
            if (init_params->gpuID >= (guint)devices && 0) {
                g_printerr("Error: Invalid gpu device ID (%d). CUDA device count (%d)\n",
            ...
            init_params->maxBatchSize = g_key_file_get_integer(
                key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_BATCH_SIZE, &error);
            CHECK_ERROR(error);
            ...
            init_params->workspaceSize = g_key_file_get_integer(
                key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_WORKSPACE_SIZE, &error);
            CHECK_ERROR(error);
            ...
            init_params->inferInputDims = NvDsInferDimsCHW{
                (unsigned int)int_list[0], (unsigned int)int_list[1], (unsigned int)int_list[2]};
            g_free(int_list);
            ...
            init_params->numDetectedClasses = g_key_file_get_integer(
                key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_NUM_DETECTED_CLASSES, &error);
            CHECK_ERROR(error);
            ...
            if (g_key_file_has_key(key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_CLUSTER_MODE,
                                   nullptr)) {
                g_printerr(
            ...
            init_params->classifierThreshold = g_key_file_get_double(
                key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_CLASSIFIER_THRESHOLD, &error);
            CHECK_ERROR(error);
            ...
            init_params->segmentationThreshold = g_key_file_get_double(
                key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_SEGMENTATION_THRESHOLD, &error);
            CHECK_ERROR(error);
                                       ...
                                       const gchar *cfg_file_path)
{
    ...
    if (init_params->networkType == NvDsInferNetworkType_Detector ||
        ...
        if (g_key_file_has_group(cfg_file, CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX "all")) {
            ...
            g_key_file_remove_group(cfg_file, CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX "all", nullptr);
        }
        ...
        nvinfer->perClassDetectionFilterParams = new std::vector<GstNvInferDetectionFilterParams>(
            init_params->numDetectedClasses, detection_filter_params);
        nvinfer->perClassColorParams =
            new std::vector<GstNvInferColorParams>(init_params->numDetectedClasses, color_params);
        ...
        for (group = groups; *group; group++) {
            if (!strncmp(*group, CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX,
                ...
                /* Parse the group. */
                if (!gst_nvinfer_parse_class_attrs(
                        cfg_file, *group, init_params->perClassDetectionParams[class_index],
