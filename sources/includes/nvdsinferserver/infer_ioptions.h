...
#define OPTION_NVDS_BUF_SURFACE_PARAMS_LIST \
    "nvds_buf_surface_params_list"   // vector<NvBufSurfaceParams*>
...
protected:
    virtual NvDsInferStatus getValuePtr(const std::string &name,
                                        OptionType t,
                                        void *&ptr) const = 0;
    virtual NvDsInferStatus getArraySize(const std::string &key, uint32_t &size) const = 0;
    virtual NvDsInferStatus getRawPtrArray(const std::string &name,
                                           OptionType ot,
                                           void **ptrBase,
