...
public:
     ...
     * @return NVDSINFER_SUCCESS if run good.
     */
    virtual NvDsInferStatus run(SharedIBatchArray input, InferOutputCb outputCb) = 0;
 ...
 */
INFER_EXPORT_API nvdsinferserver::IInferContext *createInferTrtISContext(const char *configStr,
...
NvDsTritonServerInit(nvdsinferserver::ITritonServerInstance **instance,
                     const char *configStr,
                     uint32_t configStrLen);
...
INFER_EXPORT_API nvdsinferserver::SharedIBatchBuffer NvDsInferServerCreateStrBuf(
    ...
    const nvdsinferserver::InferDims &dims,
    uint32_t batchSize,
