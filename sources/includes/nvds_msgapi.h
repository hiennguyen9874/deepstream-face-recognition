 ...
 */
typedef enum { NVDS_MSGAPI_OK, NVDS_MSGAPI_ERR, NVDS_MSGAPI_UNKNOWN_TOPIC } NvDsMsgApiErrorType;
 ...
 * @param[in]  user_ptr              pointer passed during subscribe() for
 * context
 */
 ...
 * @param[in] topics                    Array of topics to subscribe for
 * messages
 * @param[in] num_topics                num of topics
 * @param[in] cb                        A pointer to a callback function for
 * notifying the DS event handler
 * @param[in] user_ctx                  user ptr to be passed to callback for
 * context
 *
...
/**
 * Fetch the connection signature by parsing broker_connection string and cfg
 * file
 *
 * A connection signature is a unique string used to identify a connection.
 * It is generated by parsing all the connection params provided in broker_str
 * and cfg file
 *
 * Connection signature can be retreived only if the cfg option share-connection
 * = 1
 *
 ...
 *                        Empty string("") in case of errors or if
 * share-connection cfg option is not set to 1
 */
