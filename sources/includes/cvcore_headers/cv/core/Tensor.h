...
public:
     ...
     * @param isCPU whether to allocate tensor on CPU or GPU.
     */
    Tensor2D(std::initializer_list<DimData> dimData, bool isCPU) : TensorBase(CT, dimData, isCPU) {}
     ...
     */
    std::size_t getLength() const { return getSize(DimToIndex2D<TL>::kLength); }

    /**
     * Get the channel count of the 2D tensor.
     * @return channel count of the 2D tensor.
     */
    std::size_t getChannelCount() const { return getSize(DimToIndex2D<TL>::kChannel); }
     ...
     */
    DataType *getData() { return reinterpret_cast<DataType *>(TensorBase::getData()); }

    /**
     * Get the const raw data pointer to the 2D tensor.
     * @return const data pointer to the 2D tensor.
     */
    const DataType *getData() const { return reinterpret_cast<DataType *>(TensorBase::getData()); }
};
...
public:
     ...
     * @param isCPU whether to allocate tensor on CPU or GPU.
     */
    Tensor3D(std::initializer_list<DimData> dimData, bool isCPU) : TensorBase(CT, dimData, isCPU) {}
     ...
     */
    std::size_t getWidth() const { return getSize(DimToIndex3D<TL>::kWidth); }

    /**
     * Get the height of the 3D tensor.
     * @return height of the 3D tensor.
     */
    std::size_t getHeight() const { return getSize(DimToIndex3D<TL>::kHeight); }

    /**
     * Get the channel count of the 3D tensor.
     * @return channel count of the 3D tensor.
     */
    std::size_t getChannelCount() const { return getSize(DimToIndex3D<TL>::kChannel); }
...
public:
     ...
     * @param isCPU whether to allocate tensor on CPU or GPU.
     */
    Tensor4D(std::initializer_list<DimData> dimData, bool isCPU) : TensorBase(CT, dimData, isCPU) {}
     ...
     */
    std::size_t getWidth() const { return getSize(DimToIndex4D<TL>::kWidth); }

    /**
     * Get the height of the 4D tensor.
     * @return height of the 4D tensor.
     */
    std::size_t getHeight() const { return getSize(DimToIndex4D<TL>::kHeight); }

    /**
     * Get the depth of the 4D tensor.
     * @return depth of the 4D tensor.
     */
    std::size_t getDepth() const { return getSize(DimToIndex4D<TL>::kDepth); }

    /**
     * Get the channel count of the 4D tensor.
     * @return channel count of the 4D tensor.
     */
    std::size_t getChannelCount() const { return getSize(DimToIndex4D<TL>::kChannel); }
...
public:
    ...
    Tensor(std::size_t length, bool isCPU = true)
        : detail::Tensor2D<LC, CT>(
              {{length, detail::ChannelToCount<CC>()}, {detail::ChannelToCount<CC>(), 1}},
              isCPU)
    {
    }
...
public:
    ...
    template <ChannelCount T = CC, typename = typename std::enable_if<T != CX>::type>
    Tensor(std::size_t length, DataType *dataPtr, bool isCPU = true)
        : detail::Tensor2D<CL, CT>({{detail::ChannelToCount<CC>(), length}, {length, 1}},
                                   dataPtr,
                                   isCPU)
    {
    }
...
public:
    ...
    Tensor(std::size_t width, std::size_t height, std::size_t channelCount, B isCPU = true)
        : detail::Tensor3D<HWC, CT>(
              {{height, width * channelCount}, {width, channelCount}, {channelCount, 1}},
              isCPU)
    {
    }
    ...
    Tensor(std::size_t width,
           ...
           B isCPU = true)
        : detail::Tensor3D<HWC, CT>(
              {{height, rowPitch / GetChannelSize(CT)}, {width, channelCount}, {channelCount, 1}},
              dataPtr,
              isCPU)
    {
        if (rowPitch % GetChannelSize(CT) != 0) {
...
public:
    ...
    Tensor(std::size_t width, std::size_t height, bool isCPU = true)
        : detail::Tensor3D<CHW, CT>(
              {{detail::ChannelToCount<CC>(), width * height}, {height, width}, {width, 1}},
              isCPU)
    {
    }
    ...
    Tensor(std::size_t width, std::size_t height, std::size_t channelCount, bool isCPU = true)
        : detail::Tensor3D<CHW, CT>({{channelCount, width * height}, {height, width}, {width, 1}},
                                    isCPU)
    {
    }
              ...
              isCPU)
    {
        if (rowPitch % GetChannelSize(CT) != 0) {
            throw std::domain_error(
                "cvcore::Tensor<CHW, CC, CT>::Tensor ==> Parameter rowPitch is not evenly "
                "divisible by channel size");
        }
    }
...
public:
    ...
    Tensor(std::size_t width,
           ...
           std::size_t channelCount,
           bool isCPU = true)
        : detail::Tensor4D<DCHW, CT>({{depth, channelCount * width * height},
    ...
    Tensor(std::size_t width,
        ...
        : detail::Tensor4D<DCHW, CT>(
              {{depth, detail::ChannelToCount<CC>() * height * rowPitch / GetChannelSize(CT)},
               {detail::ChannelToCount<CC>(), height * rowPitch / GetChannelSize(CT)},
               {height, rowPitch / GetChannelSize(CT)},
               {width, 1}},
              dataPtr,
              isCPU)
    {
        if (rowPitch % GetChannelSize(CT) != 0) {
            throw std::domain_error(
                "cvcore::Tensor<DCHW, CC, CT>::Tensor ==> Parameter rowPitch is not evenly "
                "divisible by channel size");
        }
    }
    ...
    Tensor(std::size_t width,
           ...
           B isCPU = true)
        : detail::Tensor4D<DCHW, CT>(
              {{depth, channelCount * height * rowPitch / GetChannelSize(CT)},
...
public:
    ...
    Tensor(std::size_t width,
           ...
           std::size_t channelCount,
           bool isCPU = true)
        : detail::Tensor4D<CDHW, CT>({{channelCount, depth * width * height},
                                      {depth, width * height},
                                      {height, width},
                                      {width, 1}},
                                     isCPU)
