...
DlLibHandle::DlLibHandle(const std::string &path, int mode) : m_LibPath(path)
{
    ...
    if (!m_LibHandle) {
        dsInferError("Could not open lib: %s, error string: %s", path.c_str(), dlerror());
    }
}
...
void normalizeDims(NvDsInferDims &dims)
{
    if (hasWildcard(dims) || !dims.numDims) {
        ...
        dims.numElements =
            std::accumulate(dims.d, dims.d + dims.numDims, 1, [](int s, int i) { return s * i; });
    }
}
...
{
    return std::any_of(dims.d, dims.d + dims.numDims, [](int d) { return d == -1; });
}
...
nvinfer1::DataType str2PrecisionType(const std::string &dataType)
{
    if (!dataType.compare("fp32"))
        ...
        dsInferError("Invalid precisionType string %s. Using default kFLOAT(fp32) precisonType",
                     dataType.c_str());
...
__attribute__((visibility("default"))) const char *NvDsInferStatus2Str(NvDsInferStatus status)
{
