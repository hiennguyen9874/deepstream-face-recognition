...
public:
    InferPreprocessor(const NvDsInferNetworkInfo &info,
...
private:
    ...
    virtual NvDsInferStatus parseEachBatch(const std::vector<NvDsInferLayerInfo> &outputLayers,
...
private:
    ...
    void preClusteringThreshold(NvDsInferParseDetectionParams const &detectionParams,
                                std::vector<NvDsInferObjectDetectionInfo> &objectList);
    void filterTopKOutputs(const int topK, std::vector<NvDsInferObjectDetectionInfo> &objectList);
...
private:
    ...
    void filterTopKOutputs(const int topK, std::vector<NvDsInferInstanceMaskInfo> &objectList);
...
private:
    ...
    NvDsInferStatus initInferenceInfo(const NvDsInferContextInitParams &initParams,
                                      BackendContext &ctx);
    NvDsInferStatus preparePreprocess(const NvDsInferContextInitParams &initParams);
    NvDsInferStatus preparePostprocess(const NvDsInferContextInitParams &initParams);

    std::unique_ptr<BackendContext> generateBackendContext(NvDsInferContextInitParams &initParams);
    std::unique_ptr<BackendContext> buildModel(NvDsInferContextInitParams &initParams);
    bool deserializeEngineAndBackend(const std::string enginePath,
                                     int dla,
                                     std::shared_ptr<TrtEngine> &engine,
                                     std::unique_ptr<BackendContext> &backend);
    NvDsInferStatus checkBackendParams(BackendContext &ctx,
...
#define printMsg(level, tag_str, fmt, ...)                                                 \
    do {                                                                                   \
        ...
        snprintf(logMsgBuffer, 5 * _MAX_STR_LENGTH,                                        \
                 tag_str " NvDsInferContextImpl::%s() <%s:%d> [UID = %d]: " fmt, __func__, \
                 baseName, __LINE__, m_UniqueID, ##__VA_ARGS__);                           \
        if (m_LoggingFunc) {                                                               \
