...
__global__ void NvDsInferConvert_CxToP3FloatKernel(float *outBuffer,
...
__global__ void NvDsInferConvert_CxToL3FloatKernel(float *outBuffer,
                                                   unsigned char *inBuffer,
                                                   unsigned int width,
                                                   unsigned int height,
                                                   unsigned int pitch,
...
__global__ void NvDsInferConvert_CxToP3FloatKernelWithMeanSubtraction(float *outBuffer,
                                                                      unsigned char *inBuffer,
                                                                      unsigned int width,
                                                                      unsigned int height,
                                                                      unsigned int pitch,
...
__global__ void NvDsInferConvert_CxToL3FloatKernelWithMeanSubtraction(float *outBuffer,
                                                                      unsigned char *inBuffer,
                                                                      unsigned int width,
                                                                      unsigned int height,
                                                                      unsigned int pitch,
...
__global__ void NvDsInferConvert_CxToP3RFloatKernel(float *outBuffer,
                                                    unsigned char *inBuffer,
                                                    unsigned int width,
                                                    unsigned int height,
                                                    unsigned int pitch,
...
__global__ void NvDsInferConvert_CxToL3RFloatKernel(float *outBuffer,
                                                    unsigned char *inBuffer,
                                                    unsigned int width,
                                                    unsigned int height,
                                                    unsigned int pitch,
...
__global__ void NvDsInferConvert_CxToP3RFloatKernelWithMeanSubtraction(float *outBuffer,
                                                                       unsigned char *inBuffer,
                                                                       unsigned int width,
                                                                       unsigned int height,
                                                                       unsigned int pitch,
...
__global__ void NvDsInferConvert_CxToL3RFloatKernelWithMeanSubtraction(float *outBuffer,
                                                                       unsigned char *inBuffer,
                                                                       unsigned int width,
                                                                       unsigned int height,
                                                                       unsigned int pitch,
...
__global__ void NvDsInferConvert_C1ToP1FloatKernel(float *outBuffer,
                                                   unsigned char *inBuffer,
                                                   unsigned int width,
                                                   unsigned int height,
                                                   unsigned int pitch,
...
__global__ void NvDsInferConvert_C1ToP1FloatKernelWithMeanSubtraction(float *outBuffer,
                                                                      unsigned char *inBuffer,
                                                                      unsigned int width,
                                                                      unsigned int height,
                                                                      unsigned int pitch,
                                                                      ...
                                                                      float *meanDataBuffer)
{
    ...
    if (col < width && row < height) {
        outBuffer[row * width + col] = scaleFactor * ((float)inBuffer[row * pitch + col] -
                                                      meanDataBuffer[(row * width) + col]);
    }
}

__global__ void NvDsInferConvert_FtFTensorKernel(float *outBuffer,
                                                 float *inBuffer,
                                                 unsigned int width,
                                                 unsigned int height,
                                                 unsigned int pitch,
...
__global__ void NvDsInferConvert_FtFTensorKernelWithMeanSubtraction(float *outBuffer,
                                                                    float *inBuffer,
                                                                    unsigned int width,
                                                                    unsigned int height,
                                                                    unsigned int pitch,
                                                                    ...
                                                                    float *meanDataBuffer)
{
    ...
    if (col < width && row < height) {
        outBuffer[row * width + col] = scaleFactor * ((float)inBuffer[row * width + col] -
                                                      meanDataBuffer[(row * width) + col]);
    }
}
                                  ...
                                  cudaStream_t stream)
{
    ...
    if (meanDataBuffer == NULL) {
        ...
        NvDsInferConvert_CxToP3FloatKernelWithMeanSubtraction<<<blocks, threadsPerBlock, 0,
                                                                stream>>>(
                                  ...
                                  cudaStream_t stream)
{
    ...
    if (meanDataBuffer == NULL) {
        ...
        NvDsInferConvert_CxToL3FloatKernelWithMeanSubtraction<<<blocks, threadsPerBlock, 0,
                                                                stream>>>(
            outBuffer, inBuffer, width, height, pitch, 3, scaleFactor, meanDataBuffer);
    }
}
                                   ...
                                   cudaStream_t stream)
{
    ...
    if (meanDataBuffer == NULL) {
        ...
        NvDsInferConvert_CxToP3RFloatKernelWithMeanSubtraction<<<blocks, threadsPerBlock, 0,
                                                                 stream>>>(
            outBuffer, inBuffer, width, height, pitch, 3, scaleFactor, meanDataBuffer);
    }
}
                                   ...
                                   cudaStream_t stream)
{
    ...
    if (meanDataBuffer == NULL) {
        ...
        NvDsInferConvert_CxToL3RFloatKernelWithMeanSubtraction<<<blocks, threadsPerBlock, 0,
                                                                 stream>>>(
            outBuffer, inBuffer, width, height, pitch, 3, scaleFactor, meanDataBuffer);
    }
}
                                  ...
                                  cudaStream_t stream)
{
    ...
    if (meanDataBuffer == NULL) {
        ...
        NvDsInferConvert_C1ToP1FloatKernelWithMeanSubtraction<<<blocks, threadsPerBlock, 0,
                                                                stream>>>(
