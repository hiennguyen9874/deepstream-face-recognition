...
int main()
{
    ...
    NvDsMsgApiErrorType (*msgapi_send_ptr)(NvDsMsgApiHandle conn, char *topic,
                                           const uint8_t *payload, size_t nbuf);
    NvDsMsgApiErrorType (*msgapi_send_async_ptr)(
        NvDsMsgApiHandle h_ptr, char *topic, const uint8_t *payload, size_t nbuf,
        nvds_msgapi_send_cb_t send_callback, void *user_ptr);
    NvDsMsgApiErrorType (*msgapi_subscribe_ptr)(
        NvDsMsgApiHandle conn, char **topics, int num_topics, nvds_msgapi_subscribe_request_cb_t cb,
        void *user_ctx);
    ...
    NvDsMsgApiErrorType (*msgapi_connection_signature_ptr)(char *connection_str, char *config_path,
    ...
    *(void **)(&msgapi_connection_signature_ptr) =
    ...
    if (msgapi_connection_signature_ptr((char *)KAFKA_CONNECT_STR, (char *)KAFKA_CFG_FILE,
    ...
    conn_handle = msgapi_connect_ptr((char *)KAFKA_CONNECT_STR,
    ...
    int num_topics = 2;
    if (msgapi_subscribe_ptr(conn_handle, (char **)topics, num_topics, subscribe_cb,
                             &consumed_cnt) != NVDS_MSGAPI_OK) {
    ...
    for (int i = 0; i < 5; i++) {
        if (msgapi_send_async_ptr(conn_handle, (char *)"topic1", (const uint8_t *)SEND_MSG,
                                  strlen(SEND_MSG), test_send_cb,
                                  &(display_str[i][0])) != NVDS_MSGAPI_OK)
