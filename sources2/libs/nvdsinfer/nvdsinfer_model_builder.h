...
public:
    OnnxModelParser(const NvDsInferContextInitParams &initParams,
                    const std::shared_ptr<DlLibHandle> &handle = nullptr)
        : BaseModelParser(initParams, handle), m_ModelName(initParams.onnxFilePath)
    {
    }
...
public:
    ...
    NvDsInferStatus parseModel(nvinfer1::INetworkDefinition &network) override;
    bool hasFullDimsSupported() const override { return m_CustomParser->hasFullDimsSupported(); }
...
public:
    ...
    NvDsInferStatus getFullDimsLayersInfo(int profileIdx,
...
public:
     ...
     */
    std::unique_ptr<TrtEngine> buildModel(const NvDsInferContextInitParams &initParams,
...
private:
    ...
    std::unique_ptr<BuildParams> createImplicitParams(const NvDsInferContextInitParams &initParams);
    std::unique_ptr<BuildParams> createDynamicParams(const NvDsInferContextInitParams &initParams);
    void initCommonParams(BuildParams &params, const NvDsInferContextInitParams &initParams);
