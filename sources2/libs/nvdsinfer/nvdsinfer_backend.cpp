...
TrtBackendContext::TrtBackendContext(UniquePtrWDestroy<nvinfer1::IExecutionContext> &&ctx,
...
std::unique_ptr<TrtBackendContext> createBackendContext(const std::shared_ptr<TrtEngine> &engine)
{
    ...
    UniquePtrWDestroy<nvinfer1::IExecutionContext> cudaCtx((*engine)->createExecutionContext());
    ...
    if (!(*engine)->hasImplicitBatchDimension()) {
        ...
        if (engine->hasDla()) {
            backend = std::make_unique<DlaImplicitTrtBackendContext>(std::move(cudaCtx), engine);
        } else {
            backend = std::make_unique<ImplicitTrtBackendContext>(std::move(cudaCtx), engine);
        }
    }
...
bool ImplicitTrtBackendContext::canSupportBatchDims(int bindingIdx,
...
NvDsInferStatus FullDimTrtBackendContext::initialize()
{
    ...
    for (int i = 0; i < (int)(*m_CudaEngine)->getNbBindings(); i++) {
        ...
        nvinfer1::Dims optDims =
            (*m_CudaEngine)
                ->getProfileDimensions(i, m_ProfileIndex, nvinfer1::OptProfileSelector::kOPT);
