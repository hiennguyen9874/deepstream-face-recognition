...
static void returnInputCudaCallback(cudaStream_t stream, cudaError_t status, void *userData)
{
...
InferPreprocessor::InferPreprocessor(const NvDsInferNetworkInfo &info,
                                     ...
                                     int id)
    : m_UniqueID(id), m_NetworkInfo(info), m_NetworkInputFormat(format),
...
NvDsInferStatus InferPreprocessor::readMeanImageFile()
{
    ...
    cudaReturn = cudaMemcpy(m_MeanDataBuffer->ptr(), tempMeanDataFloat, size * sizeof(float),
...
NvDsInferStatus InferPreprocessor::allocateResource()
{
    if (!m_MeanFile.empty() || m_ChannelMeans.size() > 0) {
         ...
         * memory. */
        m_MeanDataBuffer =
            std::make_unique<CudaDeviceBuffer>((size_t)m_NetworkInfo.width * m_NetworkInfo.height *
    ...
    /* Create the mean data buffer from per-channel offsets. */
        ...
        cudaError_t cudaReturn =
            cudaMemcpy(m_MeanDataBuffer->ptr(), meanData.data(), meanData.size() * sizeof(float),
                       cudaMemcpyHostToDevice);
        if (cudaReturn != cudaSuccess) {
    ...
    std::string nvtx_name = "nvdsinfer_preprocess_uid=" + std::to_string(m_UniqueID);
    ...
    m_PreProcessCompleteEvent = std::make_shared<CudaEvent>(cudaEventDisableTiming);
...
NvDsInferStatus InferPreprocessor::transform(NvDsInferContextBatchInput &batchInput,
                                             void *devBuf,
                                             ...
                                             CudaEvent *waitingEvent)
{
    ...
    for (unsigned int i = 0; i < batchSize; i++) {
        ...
        if (convertFcn) {
            ...
            convertFcn(outPtr, (unsigned char *)batchInput.inputFrames[i], m_NetworkInfo.width,
            ...
            /* Input needs to be pre-processed. */
            convertFcnFloat(outPtr, (float *)batchInput.inputFrames[i], m_NetworkInfo.width,
                            m_NetworkInfo.height, batchInput.inputPitch, m_Scale,
                            m_MeanDataBuffer.get() ? m_MeanDataBuffer->ptr<float>() : nullptr,
                            *m_PreProcessStream);
        }
        ...
        RETURN_CUDA_ERR(
            cudaStreamAddCallback(
                *m_PreProcessStream, returnInputCudaCallback,
                new NvDsInferReturnInputPair(batchInput.returnInputFunc, batchInput.returnFuncData),
                0),
     ...
     * kernels. */
    RETURN_CUDA_ERR(cudaEventRecord(*m_PreProcessCompleteEvent, *m_PreProcessStream),
                    "Failed to record cuda event");

    RETURN_CUDA_ERR(cudaStreamWaitEvent(mainStream, *m_PreProcessCompleteEvent, 0),
 ...
 */
NvDsInferStatus InferPostprocessor::parseLabelsFile(const std::string &labelsFilePath)
{
...
NvDsInferStatus InferPostprocessor::initResource(const NvDsInferContextInitParams &initParams)
{
    ...
    RETURN_NVINFER_ERROR(allocDeviceResource(), "allocate device resource failed");
    return NVDSINFER_SUCCESS;
}

NvDsInferStatus InferPostprocessor::copyBuffersToHostMemory(NvDsInferBatch &batch,
                                                            CudaStream &mainStream)
{
...
NvDsInferStatus InferPostprocessor::postProcessHost(NvDsInferBatch &batch,
...
NvDsInferStatus DetectPostprocessor::initResource(const NvDsInferContextInitParams &initParams)
{
    ...
    if (m_CustomLibHandle && !string_empty(initParams.customBBoxParseFuncName)) {
        m_CustomBBoxParseFunc =
            m_CustomLibHandle->symbol<NvDsInferParseCustomFunc>(initParams.customBBoxParseFuncName);
    ...
    const NvDsInferContextInitParams &initParams)
{
    ...
    /* If custom parse function is specified get the function address from the
     * custom library. */
    if (m_CustomLibHandle && !string_empty(initParams.customBBoxInstanceMaskParseFuncName)) {
        m_CustomParseFunc = m_CustomLibHandle->symbol<NvDsInferInstanceMaskParseCustomFunc>(
...
NvDsInferStatus ClassifyPostprocessor::initResource(const NvDsInferContextInitParams &initParams)
{
    ...
    /* If custom parse function is specified get the function address from the
     * custom library. */
    if (m_CustomLibHandle && !string_empty(initParams.customClassifierParseFuncName)) {
...
NvDsInferStatus SegmentPostprocessor::initResource(const NvDsInferContextInitParams &initParams)
{
    RETURN_NVINFER_ERROR(InferPostprocessor::initResource(initParams),
                         "init post processing resource failed");
...
NvDsInferStatus OtherPostprocessor::initResource(const NvDsInferContextInitParams &initParams)
{
    return NVDSINFER_SUCCESS;
}
    ...
    const NvDsInferContextInitParams &initParams)
{
    ...
    std::unique_ptr<InferPreprocessor> processor = std::make_unique<InferPreprocessor>(
        m_NetworkInfo, initParams.networkInputFormat, m_InputImageLayerInfo, m_UniqueID);
    assert(processor);

    processor->setLoggingFunc(m_LoggingFunc);

    if (initParams.networkScaleFactor > 0.0f) {
        std::vector<float> offsets(initParams.offsets, initParams.offsets + initParams.numOffsets);
    ...
    if (!string_empty(initParams.meanImageFilePath) &&
        ...
        printError("Cannot access mean image file '%s'", safeStr(initParams.meanImageFilePath));
        return NVDSINFER_CONFIG_FAILED;
    }
 ...
 * engine. */
NvDsInferStatus NvDsInferContextImpl::initialize(NvDsInferContextInitParams &initParams,
                                                 void *userCtx,
                                                 NvDsInferContextLoggingFunc logFunc)
{
    ...
    if (initParams.clusterMode == NVDSINFER_CLUSTER_GROUP_RECTANGLES &&
        ...
        for (unsigned int i = 0;
             i < initParams.numDetectedClasses && initParams.perClassDetectionParams; i++) {
            initParams.perClassDetectionParams[i].topK = 20;
            initParams.perClassDetectionParams[i].nmsIOUThreshold = 0.5;
        }
        printWarning(
            "Warning, OpenCV has been deprecated. Using NMS for clustering instead of "
            "cv::groupRectangles with topK = 20 and NMS Threshold = 0.5");
    }
#endif
     ...
     * of this constructor being called from different threads. */
    {
        ...
        std::call_once(pluginInitFlag, [this]() { initLibNvInferPlugins(gTrtLogger.get(), ""); });
    }
    ...
    if (initParams.inputDims.c != 0 || initParams.inputDims.h != 0 || initParams.inputDims.w != 0) {
    ...
    if (initParams.numOutputIOFormats > 0 && initParams.outputIOFormats == nullptr) {
        printError("numOutputIOFormats >0 but outputIOFormats array not specified");
        return NVDSINFER_CONFIG_FAILED;
    }

    if (initParams.numLayerDevicePrecisions > 0 && initParams.layerDevicePrecisions == nullptr) {
    ...
    RETURN_CUDA_ERR(cudaSetDevice(m_GpuID), "Failed to set cuda device (%d).", m_GpuID);
...
NvDsInferStatus NvDsInferContextImpl::allocateBuffers()
{
    ...
    for (size_t iL = 0; iL < m_AllLayerInfo.size(); iL++) {
        ...
        size_t size = m_MaxBatchSize * layerDims.numElements * getElementSize(layerInfo.dataType);
...
NvDsInferStatus NvDsInferContextImpl::initNonImageInputLayers()
{
    ...
    auto initInputFcn = READ_SYMBOL(m_CustomLibHandle, NvDsInferInitializeInputLayers);
    ...
    for (size_t i = 0; i < inputLayers.size(); i++) {
        cudaReturn =
            cudaMemcpyAsync(m_BindingBuffers[inputLayers[i].bindingIndex], initBuffers[i].data(),
                            initBuffers[i].size(), cudaMemcpyHostToDevice, *m_InferStream);
        if (cudaReturn != cudaSuccess) {
...
public:
    BackendBatchBuffer(std::vector<void *> &bindings,
                       std::vector<NvDsInferBatchDimsLayerInfo> &info,
                       int batch)
...
NvDsInferStatus NvDsInferContextImpl::queueInputBatch(NvDsInferContextBatchInput &batchInput)
{
    ...
    RETURN_NVINFER_ERROR(m_Preprocessor->transform(batchInput, m_BindingBuffers[INPUT_LAYER_INDEX],
     ...
     * completion event. */
    RETURN_CUDA_ERR(cudaStreamWaitEvent(*m_PostprocessStream, *m_InferCompleteEvent, 0),
                    "postprocessing cuda waiting event failed ");
    RETURN_NVINFER_ERROR(
        m_Postprocessor->copyBuffersToHostMemory(*safeRecyleBatch, *m_PostprocessStream),
...
NvDsInferStatus NvDsInferContextImpl::queueInputBatchPreprocessed(
    NvDsInferContextBatchPreprocessedInput &batchInput)
{
    assert(m_Initialized);
    ...
    for (unsigned int i = 0; i < batchInput.numInputTensors; i++) {
        for (auto &layerInfo : allLayerInfo) {
            if (strcmp(layerInfo.layerName, batchInput.tensors[i].layerName) == 0) {
                ...
                memcpy(layerInfo.dims.d, batchInput.tensors[i].dims.d + 1,
                       layerInfo.dims.numDims * sizeof(unsigned int));
            }
        }
    }
    ...
    if (batchInput.returnInputFunc) {
        RETURN_CUDA_ERR(
            cudaStreamAddCallback(
                *m_InferStream, returnInputCudaCallback,
                new NvDsInferReturnInputPair(batchInput.returnInputFunc, batchInput.returnFuncData),
                0),
            "Failed to add cudaStream callback for returning input buffers");
    }
...
NvDsInferStatus NvDsInferContextImpl::dequeueOutputBatch(NvDsInferContextBatchOutput &batchOutput)
{
    ...
    std::unique_ptr<NvDsInferBatch, decltype(recyleFunc)> recyleBatch(m_ProcessBatchQueue.pop(),
                                                                      recyleFunc);
    assert(recyleBatch);

    /* Set the cuda device */
    RETURN_CUDA_ERR(cudaSetDevice(m_GpuID), "dequeue buffer failed to set cuda device(%s)",
                    m_GpuID);
    ...
    assert(m_Postprocessor);
    /* Fill the host buffers information in the output. */
    RETURN_NVINFER_ERROR(m_Postprocessor->postProcessHost(*recyleBatch, batchOutput),
    ...
    const NvDsInferContextInitParams &initParams)
{
    ...
    if (maxDims.batchSize < (int)initParams.maxBatchSize) {
        printWarning("Backend has maxBatchSize %d whereas %d has been requested", maxDims.batchSize,
    ...
    if (initParams.inferInputDims.c && initParams.inferInputDims.h && initParams.inferInputDims.w) {
        NvDsInferDims inputDims = {
            3,
            {initParams.inferInputDims.c, initParams.inferInputDims.h, initParams.inferInputDims.w},
            0};
        ...
        NvDsInferBatchDims requestBatchDims{(int)initParams.maxBatchSize, inputDims};
        if (!ctx.canSupportBatchDims(INPUT_LAYER_INDEX, requestBatchDims)) {
            printWarning("backend can not support dims:%s", safeStr(dims2Str(inputDims)));
            return NVDSINFER_CONFIG_FAILED;
        }
    }
                                                       ...
                                                       std::unique_ptr<BackendContext> &backend)
{
    ...
    if (!newEngine) {
        printWarning("deserialize engine from file :%s failed", safeStr(enginePath));
        return false;
    }
    ...
    NvDsInferContextInitParams &initParams)
{
    ...
    std::unique_ptr<TrtModelBuilder> builder =
        std::make_unique<TrtModelBuilder>(initParams.gpuID, *gTrtLogger, m_CustomLibHandle);
    assert(builder);

    if (!string_empty(initParams.int8CalibrationFilePath) &&
        file_accessible(initParams.int8CalibrationFilePath)) {
        auto calibrator =
            std::make_unique<NvDsInferInt8Calibrator>(initParams.int8CalibrationFilePath);
        builder->setInt8Calibrator(std::move(calibrator));
    }

    std::string enginePath;
    std::shared_ptr<TrtEngine> engine = builder->buildModel(initParams, enginePath);

    /////////////////
    /* Start Custom */
    /////////////////
    if (!string_empty(initParams.modelEngineFilePath)) {
        printInfo("Overwrite model-engine-file");
        enginePath = initParams.modelEngineFilePath;
    }
    ////////////////
    /* End Custom */
    ////////////////
    ...
    if (builder->serializeEngine(enginePath, engine->engine()) != NVDSINFER_SUCCESS) {
        printWarning("failed to serialize cude engine to file: %s", safeStr(enginePath));
    } else {
        printInfo("serialize cuda engine to file: %s successfully", safeStr(enginePath));
    }
    ...
    NvDsInferContextInitParams &initParams)
{
    ...
    std::unique_ptr<BackendContext> backend;
    if (!string_empty(initParams.modelEngineFilePath)) {
        if (!deserializeEngineAndBackend(initParams.modelEngineFilePath, dla, engine, backend)) {
            printWarning(
    ...
    if (backend && checkBackendParams(*backend, initParams) == NVDSINFER_SUCCESS) {
        printInfo("Use deserialized engine model: %s", safeStr(initParams.modelEngineFilePath));
        return backend;
    } else if (backend) {
        printWarning(
...
NvDsInferContextImpl::~NvDsInferContextImpl()
{
    ...
    if (cudaReturn != cudaSuccess) {
        printError("Failed to set cuda device %d (%s).", m_GpuID, cudaGetErrorName(cudaReturn));
        return;
    }
