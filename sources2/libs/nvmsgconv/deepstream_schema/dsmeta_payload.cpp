                                                 ...
                                                 NvDsObjectMeta *obj_meta)
{
    ...
    for (NvDsMetaList *l_user = obj_meta->obj_user_meta_list; l_user != NULL;
        ...
        if (user_meta->base_meta.meta_type == NVDSINFER_TENSOR_OUTPUT_META) {
            ...
            if (meta->unique_id == 3) {
                ...
                for (unsigned int c = 0; c < featureDim; c++) {
                    if (isinf(outputCoverageBuffer[c])) {
                        // TODO: Wanrnings this is error
                    }
                    json_array_add_double_element(feature_vector, outputCoverageBuffer[c]);
                }
                json_object_set_array_member(objectObj, "reidFeatureVector", feature_vector);
            } else {
                // TODO: Warnings not implemented
            }
            break;
        ...
        if (user_meta->base_meta.meta_type == NVDS_MASK_CROP_OBJECT_USER_OBJECT_META) {
            // FilePath from ds-example
            json_object_set_string_member(objectObj, "maskFilePath",
                                          (gchar *)user_meta->user_meta_data);
        }

        if (user_meta->base_meta.meta_type == NVDS_COLOR_POST_PROCESSING2_USER_OBJECT_CENTER_META) {
            NvDsCustomMsgInfo *custom_blob = (NvDsCustomMsgInfo *)user_meta->user_meta_data;

            int **message_centers = (int **)custom_blob->message;

            JsonArray *centers = json_array_new();
            for (size_t i = 0; i < custom_blob->size; i++) {
                JsonArray *center = json_array_new();
                json_array_add_int_element(center, message_centers[i][0]);
                json_array_add_int_element(center, message_centers[i][1]);
                json_array_add_int_element(center, message_centers[i][2]);
                json_array_add_array_element(centers, center);
            }
            json_object_set_array_member(objectObj, "centers", centers);
        }

        if (user_meta->base_meta.meta_type == NVDS_COLOR_POST_PROCESSING2_USER_OBJECT_COUTER_META) {
            NvDsCustomMsgInfo *custom_blob = (NvDsCustomMsgInfo *)user_meta->user_meta_data;

            int *message_couters = (int *)custom_blob->message;

            JsonArray *couters = json_array_new();
            for (size_t i = 0; i < custom_blob->size; i++) {
                json_array_add_int_element(couters, message_couters[i]);
            }
            json_object_set_array_member(objectObj, "couters", couters);
        }

        if (user_meta->base_meta.meta_type == NVDS_COLOR_POST_PROCESSING_USER_OBJECT_META) {
            NvDsCustomMsgInfo *custom_blob = (NvDsCustomMsgInfo *)user_meta->user_meta_data;

            JsonArray *post_processing = json_array_new();
            float *votes = (float *)custom_blob->message;
            for (size_t i = 0; i < custom_blob->size; i++) {
                json_array_add_double_element(post_processing, votes[i]);
            }
            json_object_set_array_member(objectObj, "colors_percent", post_processing);
        }
...
gchar *generate_dsmeta_message_minimal_custom(void *privData, void *frameMeta)
{
    ...
    for (NvDsUserMetaList *l = frame_meta->frame_user_meta_list; l; l = l->next) {
        ...
        if (frame_usermeta->base_meta.meta_type == NVDS_COLOR_POST_PROCESSING_USER_FRAME_META) {
            NvDsCustomMsgInfo *custom_blob = (NvDsCustomMsgInfo *)frame_usermeta->user_meta_data;

            int **initial_colors = (int **)custom_blob->message;

            JsonArray *initialColors = json_array_new();
            for (size_t i = 0; i < custom_blob->size; i++) {
                JsonArray *initialColor = json_array_new();
                json_array_add_int_element(initialColor, initial_colors[i][0]);
                json_array_add_int_element(initialColor, initial_colors[i][1]);
                json_array_add_int_element(initialColor, initial_colors[i][2]);
                json_array_add_array_element(initialColors, initialColor);
            }
            json_object_set_array_member(jobject, "initial_colors", initialColors);
        }
    }
